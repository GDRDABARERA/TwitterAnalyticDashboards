...
//web-socket for speach beta section

		var checkCEPStatus = function(){
    		var ws = new WebSocket("wss://52.77.25.83:9446/outputwebsocket/VoiceSocket");
		    ...
		    ws.onerror = function (error) {
		      console.log(error);
    			...
    			if(msg.event.payloadData.name == "FINISH" ){
			    var speakerImg=msg.event.payloadData.Pic; //add the token
			    var WordContainer="";
                     	    var spName= msg.event.payloadData.SpeakerName;
                            var spTitle= msg.event.payloadData.Title; //title speech
                            var spDescription=msg.event.payloadData.dis;
 									...
 									"<div class='row'>"+ 
 										"<div class='col-md-12 full-height js-full-height'>"+ 
 											"<div class='fh5co-cover-intro'>"+ 
 												"<h1 class='cover-text-lead wow fadeInUp' data-wow-duration='1s' data-wow-delay='.5s'>Speech Stats the speech</h1>"+ 
 												"<h5 class='wow fadeInUp beta-tag' data-wow-duration='1s' data-wow-delay='.9s'>Beta</h5> "+ 
 												"<div class='graph-container speech-stat'> "+ 
 													"<img class='preserve' src='"+speakerImg+"' />"+ 
 													"<div class='stat-container'>"+ 
 														"<h2>You just listened to <span class='speaker-name' >"+spName+"</span></h2> "+ 
 														"<h3 id='speakerTitle'>"+spTitle+"</h3> "+ 
 														"<p  id='speakerDescription'>"+spDescription+"</p>"+ 
 														"<div class='word-count-container'>"+WordContainer+"</div>"+ 
 													"</div> "+ 
 												"</div> "+ 
 											"</div> "+ 
 										"</div> "+ 
...
getTop5();
		...
		$('#page-container').on('animating.slides',function(e,t){
			...
			if(t.upcoming_slide == 0){
				...
				$('#CommunityGraph').css('display','block');
		   $('#CommunityGraph').width(t.upcoming_slide_width / 1.7);
